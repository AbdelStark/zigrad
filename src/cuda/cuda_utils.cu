...
extern "C" void device_synchronize() { CUDA_ASSERT(cudaDeviceSynchronize()); }
...
extern "C" void deinit_cudnn_handle(CudnnWrapper w) { CUDNN_ASSERT(cudnnDestroy(__cast_cudnn(w))); }
...
extern "C" void mem_sequence(
  dtype id, void *data, len_t n, const void *init, const void *step, StreamWrapper stream
) {
...
template <typename T> struct UniformRandom {
  unsigned seed;
  __host__ __device__ T operator()(unsigned n) const {
...
template <typename T> struct NormalRandom {
  unsigned seed;
  __host__ __device__ T operator()(unsigned n) const {
    thrust::default_random_engine rng(this->seed);
...
void __mem_random(void *x, len_t n, randtype op, unsigned seed, StreamWrapper stream) {
  ...
  if (op == UNIFORM) {
    thrust::transform(
      thrust::cuda::par.on(_stream), idxs, idxs + n, static_cast<T *>(x),
      UniformRandom<T>{.seed = seed}
    );
  } else {
    thrust::transform(
      thrust::cuda::par.on(_stream), idxs, idxs + n, static_cast<T *>(x),
      NormalRandom<T>{.seed = seed}
    );
  }
}

extern "C" void
mem_random(dtype id, void *x, len_t n, randtype op, unsigned seed, StreamWrapper stream) {
  switch (id) {
  case SINGLE:
    return __mem_random<f32>(x, n, op, seed, stream);
  case DOUBLE:
    return __mem_random<f64>(x, n, op, seed, stream);
  default:
...
template <typename T>
void __mem_take(
  const void *src, len_t src_len, const len_t *idxs, len_t idxs_len, void *dst, StreamWrapper stream
) {
  const auto _stream = __cast_stream(stream);
  ...
  thrust::transform(
    thrust::cuda::par.on(_stream), stencil, stencil + idxs_len, dst_itr,
    [=] __device__(len_t i) -> T { return src_itr[idxs[i]]; }
  );
}
...
extern "C" void mem_take(
  ...
  switch (id) {
  case SINGLE:
    return __mem_take<f32>(src, src_len, idxs, idxs_len, dst, stream);
  case DOUBLE:
    return __mem_take<f64>(src, src_len, idxs, idxs_len, dst, stream);
  default:
    SYSTEM_EXIT("Unsupported data type");
  }
}
...
struct Memmap {
  ...
  static Memmap *unwrap(MemmapWrapper wrapper) { return static_cast<Memmap *>(wrapper.ptr); }

  static MemmapWrapper wrap(Memmap *backend) { return {.ptr = backend}; }
...
extern "C" void reset_memmap(MemmapWrapper wrapper) { Memmap::unwrap(wrapper)->reset(); }

extern "C" void deinit_memmap(MemmapWrapper wrapper) { delete Memmap::unwrap(wrapper); }
...
// TODO: mem_map we should prob return the real size, mem_unmap needs to know
extern "C" void *mem_map(unsigned device_id, size_t virtual_buffer_size) {
  ...
  std::thread ctx_thread(
    [=](CUdeviceptr *dptr_ref) {
      ...
      // CURESULT_ASSERT(cuMemAddressReserve(dptr_ref, virtual_buffer_size,
      // mem_page_size(device_id), 0, 0));
...
extern "C" void mem_unmap(unsigned device_id, void *base_address, size_t virtual_buffer_size) {
  ...
  std::thread ctx_thread([=]() {
    ...
    // HACK: round again to recover the actual size we allocated
    const size_t page = static_cast<size_t>(mem_page_size(device_id));
    const size_t size = round_up(virtual_buffer_size ? virtual_buffer_size : page, page);
