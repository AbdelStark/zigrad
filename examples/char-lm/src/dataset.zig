...
pub const CharCorpus = struct {
    ...
    pub fn deinit(self: *CharCorpus) void {
        self.allocator.free(self.vocab);
        self.allocator.free(self.token_ids);
        self.* = undefined;
    }

    pub fn sampleCount(self: *const CharCorpus) usize {
        return self.token_ids.len - self.context_len;
    }

    pub fn batchCount(self: *const CharCorpus) usize {
        const samples = self.sampleCount();
        return (samples + self.batch_size - 1) / self.batch_size;
    }

    pub fn vocabSize(self: *const CharCorpus) usize {
        return self.vocab.len;
    }

    pub fn tokenForByte(self: *const CharCorpus, byte: u8) usize {
        const sentinel = std.math.maxInt(usize);
    ...
    pub fn byteForToken(self: *const CharCorpus, token: usize) u8 {
        return self.vocab[token];
    }

    pub fn makeBatch(self: *const CharCorpus, device: zg.DeviceReference, batch_index: usize) !Batch {
