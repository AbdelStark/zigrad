...
fn printUsage() void {
    std.debug.print(
        \\Usage: benchmark-validate [--spec-root <path>] [--group primitive|blas|autograd|memory|compiler|interop|model-train|model-infer|models|all] [--spec <path>] [--input <path> ...] [--json-output <path>] [--report-output <path>]
        \\
    , .{});
}
