...
const GroupBuilder = struct {
    ...
    batch_size: ?usize,
    entries: std.ArrayList(ProviderEntry),
...
pub fn buildReport(
    ...
    for (inputs) |input_file| {
        for (input_file.records) |record_entry| {
            ...
            const group_gop = try group_index.getOrPut(key);
            if (!group_gop.found_existing) {
                try group_builders.append(allocator, GroupBuilder.init(record_entry));
                group_gop.value_ptr.* = group_builders.items.len - 1;
            }

            var group_builder = &group_builders.items[group_gop.value_ptr.*];
            ...
            for (group_builder.entries.items) |existing| {
                if (std.mem.eql(u8, existing.provider, record_entry.backend.host_provider)) {
                    return error.DuplicateProviderRecord;
                }
            }
    ...
    if (summary.host_records == 0) return error.NoHostProviderRecords;
    ...
    for (group_builders.items, 0..) |*group_builder, group_idx| {
        sortEntries(group_builder.entries.items, baseline_provider);
        const baseline_available = applyBaseline(group_builder.entries.items, baseline_provider);
        if (baseline_provider != null and !baseline_available) {
            summary.baseline_missing_groups += 1;
        }
        ...
        groups[group_idx] = .{
            ...
            .thread_count = group_builder.thread_count,
...
pub fn writeMarkdownReport(
    ...
    try writer.writeAll("# Host Provider Benchmark Report\n\n");
    ...
    if (report.baseline_provider) |provider| {
        try writer.print("- Baseline provider: `{s}`\n", .{provider});
        try writer.print("- Groups missing baseline provider: {}\n", .{report.summary.baseline_missing_groups});
    }
    try writer.print("- Comparable entries with baseline deltas: {}\n", .{report.summary.comparable_entries});
    try writer.writeByte('\n');

    for (report.groups) |group| {
        try writer.print("## `{s}`\n\n", .{group.benchmark_id});
        try writer.print(
            "Suite `{s}`, kind `{s}`, dtype `{s}`, runner `{s}`",
            .{ group.suite, group.kind, group.dtype, group.runner },
        );
        if (group.thread_count) |thread_count| {
            try writer.print(", threads `{d}`", .{thread_count});
        }
        try writer.writeAll(".\n\n");

        try writer.writeAll("| Provider | Status | Mean ms | Delta vs baseline | Speedup vs baseline | Throughput | Peak live KiB | Telemetry | Notes |\n");
        try writer.writeAll("| --- | --- | ---: | ---: | ---: | --- | ---: | --- | --- |\n");
...
fn parseArgs(allocator: std.mem.Allocator) !Options {
    ...
    while (index < args.len) : (index += 1) {
        ...
        } else if (std.mem.eql(u8, arg, "--baseline-provider")) {
            options.baseline_provider = try nextArg(args, &index, "--baseline-provider");
...
fn printUsage() void {
    std.debug.print(
        \\Usage: benchmark-provider-report --input <path> [--input <path> ...] [--runner <name>] [--baseline-provider <provider>] [--markdown-output <path>] [--json-output <path>]
        \\
    , .{});
}
...
fn makeGroupKey(allocator: std.mem.Allocator, record_entry: result.Record) ![]const u8 {
    return std.fmt.allocPrint(
        ...
        .{
            ...
            record_entry.runner,
            try threadKey(allocator, record_entry.backend.thread_count),
        },
    );
}

fn threadKey(allocator: std.mem.Allocator, thread_count: ?u32) ![]const u8 {
...
fn sortEntries(entries: []ProviderEntry, baseline_provider: ?[]const u8) void {
    const Context = struct {
        baseline_provider: ?[]const u8,
    };
    std.mem.sort(ProviderEntry, entries, Context{ .baseline_provider = baseline_provider }, struct {
        fn lessThan(ctx: Context, lhs: ProviderEntry, rhs: ProviderEntry) bool {
            const lhs_is_baseline = ctx.baseline_provider != null and std.mem.eql(u8, lhs.provider, ctx.baseline_provider.?);
            const rhs_is_baseline = ctx.baseline_provider != null and std.mem.eql(u8, rhs.provider, ctx.baseline_provider.?);
            if (lhs_is_baseline != rhs_is_baseline) return lhs_is_baseline;
            return std.mem.order(u8, lhs.provider, rhs.provider) == .lt;
        }
    }.lessThan);
}

fn applyBaseline(entries: []ProviderEntry, baseline_provider: ?[]const u8) bool {
    const provider = baseline_provider orelse return false;

    var baseline_mean_ns: ?f64 = null;
    for (entries) |entry| {
        if (!std.mem.eql(u8, entry.provider, provider)) continue;
        if (entry.status != .ok or entry.mean_ns == null) return false;
        baseline_mean_ns = entry.mean_ns;
        break;
    }

    if (baseline_mean_ns == null) return false;
...
fn sortGroups(groups: []BenchmarkGroup) void {
    std.mem.sort(BenchmarkGroup, groups, {}, struct {
        fn lessThan(_: void, lhs: BenchmarkGroup, rhs: BenchmarkGroup) bool {
            ...
            const runner_order = std.mem.order(u8, lhs.runner, rhs.runner);
            if (runner_order != .eq) return runner_order == .lt;
...
fn formatDeltaCell(
    ...
    if (group.baseline_provider) |baseline_provider| {
        if (group.baseline_available and std.mem.eql(u8, entry.provider, baseline_provider)) {
            return allocator.dupe(u8, "baseline");
        }
    }
    return if (entry.delta_ratio_vs_baseline) |delta|
        try std.fmt.allocPrint(allocator, "{s}{d:.2}%", .{ signPrefix(delta), @abs(delta) * 100.0 })
    else
        allocator.dupe(u8, "n/a");
}
...
fn formatSpeedupCell(
    ...
    return if (entry.speedup_vs_baseline) |speedup|
        try std.fmt.allocPrint(allocator, "{d:.2}x", .{speedup})
    else
        allocator.dupe(u8, "n/a");
}
...
fn formatThroughput(allocator: std.mem.Allocator, entry: ProviderEntry) ![]const u8 {
...
fn formatKiB(allocator: std.mem.Allocator, value: ?u64) ![]const u8 {
    return if (value) |bytes|
        try std.fmt.allocPrint(allocator, "{d:.2}", .{@as(f64, @floatFromInt(bytes)) / 1024.0})
    else
        allocator.dupe(u8, "n/a");
}
...
fn appendTestRecord(
    ...
    try builder.writer(allocator).print(
        "{{\"benchmark_id\":\"{s}\",\"suite\":\"primitive\",\"kind\":\"primitive_add\",\"runner\":\"{s}\",\"status\":\"{s}\",\"dtype\":\"f32\",\"warmup_iterations\":1,\"measured_iterations\":2,\"batch_size\":null,\"seed\":1,\"shapes\":[{{\"name\":\"lhs\",\"dims\":[1]}}],\"runtime\":{{\"timestamp_unix_ms\":0,\"git_commit\":\"deadbeef\",\"git_dirty\":false,\"zig_version\":\"0.15.2\",\"harness_version\":\"0.1.0\"}},\"system\":{{\"os\":\"linux\",\"kernel\":\"test\",\"arch\":\"x86_64\",\"cpu_model\":\"cpu\",\"cpu_logical_cores\":1,\"total_memory_bytes\":null}},\"backend\":{{\"device\":\"{s}\",\"host_provider\":\"{s}\",\"thread_count\":{s},\"accelerator\":null,\"host_blas_telemetry\":{s}}},\"setup_latency_ns\":10,\"stats\":{s},\"memory\":{s},\"notes\":{s}}}\n",
        .{
            spec.benchmark_id,
            spec.runner,
            @tagName(spec.status),
...
test "provider report groups providers and computes baseline deltas" {
    ...
    var accelerate = try loadTestInput(allocator, &.{
        ...
        .{
            .benchmark_id = "memory.train-step",
            .provider = "accelerate",
            .mean_ns = 220_000_000.0,
    ...
    var openblas = try loadTestInput(allocator, &.{
        .{
            ...
            .telemetry = .{ .matmul_calls = 1, .fallback_bmm_dispatches = 1, .fallback_bmm_batches = 4 },
        },
        .{
            .benchmark_id = "memory.train-step",
            .provider = "openblas",
            .mean_ns = 250_000_000.0,
    ...
    try std.testing.expectEqual(@as(usize, 0), report.summary.baseline_missing_groups);
    try std.testing.expectEqualStrings("accelerate", report.groups[0].entries[0].provider);
    try std.testing.expectEqualStrings("openblas", report.groups[0].entries[1].provider);
    try std.testing.expectApproxEqAbs(@as(f64, -0.2), report.groups[1].entries[1].delta_ratio_vs_baseline.?, 1e-9);
    try std.testing.expectApproxEqAbs(@as(f64, 1.25), report.groups[1].entries[1].speedup_vs_baseline.?, 1e-9);
}

test "provider report rejects duplicate providers within a benchmark group" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();

    var first = try loadTestInput(allocator, &.{
        .{ .benchmark_id = "primitive.matmul", .provider = "accelerate" },
    });
    defer first.deinit();
...
test "provider report markdown includes baseline delta and telemetry" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    var openblas = try loadTestInput(allocator, &.{
        .{
            ...
            .mean_ns = 80_000_000.0,
            .telemetry = .{ .matmul_calls = 1, .fallback_bmm_dispatches = 1, .fallback_bmm_batches = 4 },
            .notes = "uses fallback broadcast path",
        },
    });
    defer openblas.deinit();
    ...
    try std.testing.expect(std.mem.indexOf(u8, output.items, "| `accelerate` | `ok` | 100.000 | baseline | 1.00x | 10.000 samples/s | n/a | dot=0, matvec=0, matmul=1, direct_bmm=0, fallback_bmm=0/0 | - |") != null);
    try std.testing.expect(std.mem.indexOf(u8, output.items, "| `openblas` | `ok` | 80.000 | -20.00% | 1.25x | 10.000 samples/s | n/a | dot=0, matvec=0, matmul=1, direct_bmm=0, fallback_bmm=1/4 | uses fallback broadcast path |") != null);
}
