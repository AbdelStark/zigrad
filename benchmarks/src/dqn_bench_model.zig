...
pub fn DqnBenchmarkModel(comptime T: type) type {
    return struct {
        const Self = @This();
        const Tensor = NDTensor(T);
        const nn = zg.nn(T);

        const dims = [_]usize{ 4, 128, 128, 2 };
        ...
        pub fn init(allocator: std.mem.Allocator, device: DeviceReference, graph: *zg.Graph, seed: u64) !Self {
            ...
            inline for (0..depth) |i| {
                ...
                self.weights[i] = try makeWeight(
                    ...
                    in_features,
                    seed +% @as(u64, i + 1),
                    std.fmt.comptimePrint("bench.dqn.weights.{d}", .{i}),
                );
                weight_count += 1;

                self.biases[i] = try Tensor.zeros(device, &.{out_features}, .{
                    .label = std.fmt.comptimePrint("bench.dqn.biases.{d}", .{i}),
                    .requires_grad = true,
                    .acquired = true,
                    .graph = graph,
                });
        ...
        pub fn forward(self: *Self, x: *Tensor) !*Tensor {
            ...
            flat.set_label("benchmark.dqn.flatten");
