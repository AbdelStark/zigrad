...
pub fn PendulumDynamicsModel(comptime T: type) type {
    return struct {
        const Self = @This();
            ...
            inline for (&self.weights, &self.biases, 0..) |*weight, *bias, i| {
                ...
                weight.* = if (seed) |value|
                    try makeDeterministicWeight(
                        ...
                        in_features,
                        value +% @as(u64, i + 1),
                        std.fmt.comptimePrint("pendulum.weights.{d}", .{i}),
                    )
                else
                    try Tensor.random(
                        device,
                        ...
                        .{
                            .graph = graph,
                            .label = std.fmt.comptimePrint("pendulum.weights.{d}", .{i}),
                            .requires_grad = true,
                            .acquired = true,
                        },
                    );
                ...
                bias.* = try Tensor.zeros(
                    ...
                    .{
                        .graph = graph,
                        .label = std.fmt.comptimePrint("pendulum.biases.{d}", .{i}),
                        .requires_grad = true,
                        .acquired = true,
                    },
                );
        ...
        pub fn forward(self: *Self, x: *Tensor) !*Tensor {
            ...
            flat.set_label("pendulum.flatten");
