...
pub fn main() !void {
    ...
    while (i > 0) : (i -= 1) {
        ...
        try stdout_file.print("\r", .{});
    }

    for (0..10) |j| {
        frameMarkNamed("Loop");
        std.Thread.sleep(std.time.ns_per_ms * std.crypto.random.intRangeLessThan(usize, 5, 50));
        const zone = trace(@src());
        defer zone.end();
        zone.setValue(j);
        std.Thread.sleep(std.time.ns_per_ms * std.crypto.random.intRangeLessThan(usize, 100, 200));
    }
}
